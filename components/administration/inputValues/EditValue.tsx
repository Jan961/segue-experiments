...
import { CustomInputValuesData } from "./ManageInputOptions";


interface Props {
    currentInputValue:CustomInputValuesData
    showModal: boolean
    setShowModal: React.Dispatch<React.SetStateAction<boolean>>
}
export default function EditValue({currentInputValue, showModal, setShowModal}:Props) {
//   const [showModal, setShowModal] = React.useState(false);
  ...
  const [inputs, setInputs] = useState<CustomInputValuesData>({
    Name: "",
    Value: "",
    OwnerId: null,
    Type: "",
  ...
  useEffect(() => {
    if(currentInputValue){
        setInputs({
            Name: currentInputValue.Name ?? "",
            Value: currentInputValue.Value ?? "",
            OwnerId: currentInputValue.OwnerId ?? null,
            Type: currentInputValue.Type ?? "",
    Id: currentInputValue.Id
        });  
    }
  ...
  const handleOnSubmit = async (e) => {
    ...
    axios({
      method: "PUT", 
      url: "/api/inputValues/write/",
      data: inputs,
    })
      .then((response) => {
        loggingService.logAction("customInput", "Custom Input Value Updated");
        handleServerResponse(
          true,
          "Thank you, your message has been updated." 
        );
        handleClose();
      })
      .catch((error) => {
            ...
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                  <h3 className="text-3xl font-semibold">Edit Input Value</h3>
                  <button
