...
export const formSchema = Yup.object({
  actOneUpTime: Yup.string()
    ...
    .test(
      'is-greater',
      'Act one UP time should be equal or greater than performance time',
      function (value) {
        const { performanceTime } = this.parent as { performanceTime: string }
        return (
          isEqual(
            new Date(`2023-01-01 ${value ?? '00:00'}`),
            new Date(`2023-01-01 ${performanceTime}`)
          ) ||
            isAfter(
              new Date(`2023-01-01 ${value ?? '00:00'}`),
              new Date(`2023-01-01 ${performanceTime}`)
  ...
  actOneDownTime: Yup.string()
    ...
    .test(
      'is-greater',
      'Act one DOWN time should be greater than Act one UP time',
      function (value) {
        const { actOneUpTime } = this.parent as { actOneUpTime: string }
        return isAfter(
          new Date(`2023-01-01 ${value ?? '00:00'}`),
          new Date(`2023-01-01 ${actOneUpTime}`)
        )
      }
    ),
  intervalDownTime: Yup.string()
    ...
    .test(
      'is-greater',
      'Interval DOWN time should be greater than Act one DOWN time',
      function (value) {
        const { actOneDownTime } = this.parent as { actOneDownTime: string }
        return isAfter(
          new Date(`2023-01-01 ${value ?? '00:00'}`),
          new Date(`2023-01-01 ${actOneDownTime}`)
        )
      }
    ),
  actTwoDownTime: Yup.string()
    ...
    .test(
      'is-greater',
      'Act two DOWN time should be greater than Act two UP time',
      function (value) {
        const { intervalDownTime } = this.parent as { intervalDownTime: string }
        return isAfter(
          new Date(`2023-01-01 ${value ?? '00:00'}`),
          new Date(`2023-01-01 ${intervalDownTime}`)
        )
      }
    ),
  getOutTime: Yup.string()
    ...
    .test(
      'is-greater',
      'Get out time should be greater than Act two DOWN time',
      function (value) {
        const { actTwoDownTime } = this.parent as { actTwoDownTime: string }
        return isAfter(
          new Date(`2023-01-01 ${value ?? '00:00'}`),
          new Date(`2023-01-01 ${actTwoDownTime}`)
        )
      }
    ),
