...
export default function SelectedVenues({ activeTours }: Props) {
  ...
  const downloadReport = async () => {
    ...
    fetch("/api/reports/venues", {
      method: "POST",
      body: JSON.stringify({
        tourCode: selectedTour
          ? `${selectedTour?.ShowCode}${selectedTour?.Code}`
          : null,
            ...
            <div className="relative w-auto my-6 mx-auto max-w-6xl">
              ...
              <div className="px-4 border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                  <h3 className="text-3xl font-semibold">Selected Venues</h3>
                  <button
                ...
                <form onSubmit={handleOnSubmit}>
                  ...
                  <div className="flex flex-col space-y-2 mt-4">
                    ...
                    >
                      ...
                      <option key="NOTONSALE" value="NOT ON SALE">
                        NOT ON SALE
                      </option>
                      <option
                        key="MARKETING PLANS RECEIVED"
                        value="MARKETING PLANS RECEIVED"
                      >
                        MARKETING PLANS RECEIVED
                      </option>
                      <option
                        key="MARKETING PLANS NOT RECEIVED"
                        value="MARKETING PLANS NOT RECEIVED"
                      >
                        MARKETING PLANS NOT RECEIVED
                      </option>
                      <option
                        key="CONTACT INFO RECEIVED"
                        value="CONTACT INFO RECEIVED"
                      >
                        CONTACT INFO RECEIVED
                      </option>
                      <option
                        key="CONTACT INFO NOT RECEIVED"
                        value="CONTACT INFO NOT RECEIVED"
                      >
                        CONTACT INFO NOT RECEIVED
                      </option>
                      <option
                        key="PRINT REQUIREMENTS RECEIVED"
                        value="PRINT REQUIREMENTS RECEIVED"
                      >
                        PRINT REQUIREMENTS RECEIVED
                      </option>
                      <option
                        key="PRINT REQUIREMENTS NOT RECEIVED"
                        value="PRINT REQUIREMENTS NOT RECEIVED"
                      >
                        PRINT REQUIREMENTS NOT RECEIVED
                      </option>
                    </select>
                  </div>
                  {/* footer */}
