...
const showsTabsEndpoint = (account) => `/api/shows/account/${account}`
...
export default function NewUser(){
    ...
    useEffect(() => {
         ...
         * Get Shows
         */
        fetch(showsTabsEndpoint(user.accountId))
            .then(res => res.json())
            .then(res => {
        ...
        fetch(showPermissionsEndpoint())
            ...
            .then(res => {
                ...
                for (let permissionInput of res.searchResults) {
                    for (let tab in tabs) {
                        //format is ShowId_PermissionId == 1_23
                        // @ts-ignore
                        let permissionTitle = tab.ShowId + "_" + permissionInput
                        let newPermission = {permissionTitle:{id: permissionTitle, name: permissionTitle, defaultValue: false, checked:false}}
        ...
        fetch(accountPermissionsEndpoint())
            ...
            .then(res => {
                ...
                for (let permissionInput of res.searchResults) {
                    let newPermission = {permissionTitle:{id: permissionInput, name: permissionInput, defaultValue: false, checked:false}}
                    setInputs((prevStatus) => ({...prevStatus, ...newPermission}))
    ...
    const handleOnSubmit = async (e) => {
        ...
        axios({
            ...
            .then((response) => {
                loggingService.logAction("New User", "update Account with new user: " + AccountId)
    ...
    return (
        <>
            <button
                className="mt-4 mx-auto w-1/5 bg-white text-primary-orange hover:bg-blue-400 active:bg-blue-600 font-semibold text-sm px-4 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mb-1 ease-linear transition-all duration-150"
                type="button"
                                ...
                                <form onSubmit={handleOnSubmit}>
                                    <div className="relative p-6 flex-auto">
                                        <div className="grid grid-cols-1 gap-2">
                                            <div>
                                                <p className={"text-gray-700 font-medium" }>Create New User</p>
                                                <p className={"text-gray-600 small" }>
                                                    When creating a user the user will be sent an invite to create a password
                                                </p>
                                            </div>
                                            <div>
                                                        ...
                                                        <div id={tab.Name} className="tabcontent hidden">
                                                            ...
                                                            {defaultShowPermissions.map((permission) => (
                                                                ...
                                                                <div className={"grid grid-cols-3"} >
                                                                    ...
                                                                    <input
                                                                        type={"checkbox"}
                                                                        id={`p-${tab.ShowId}_${permission.PermissionId}`}
                                                                        name={`p-${tab.ShowId}_${permission.PermissionId}`}
                                                                        onChange={handleOnChange}
                                                    ...
                                                    {defaultAccountPermissions.map((permission) => (
                                                        <div className={"grid grid-cols-3"} >
                                                            <input
                                                                ...
                                                                name={`p-${permission.PermissionId}`}
                                                                onChange={handleOnChange}
