            ...
            .then(res => {
                ...
                for (let permissionInput of res.searchResults) {
                    for (let tab in tabs) {
                        ...
                        // @ts-ignore
                        let permissionTitle = tab.ShowId.toString() + "_" + permissionInput
                ...
                for (let permissionInput of res.searchResults) {
                   // alert(JSON.stringify(permissionInput))
                    if (permissionInput.Shows === null) {
                        ...
                        let permission = permissionInput.PermissionId
                        //setInputs((prevStatus) => ({...prevStatus, permission: true}))
                        let newPermission = {permission:{id: permission.toString(), name: permission.toString(), defaultValue: false, checked:false}}
                        setInputs((prevStatus) => ({...prevStatus, ...newPermission}))
                        ...
                        let permissionTitle: string = permissionInput.Shows + "_" + permissionInput.PermissionId

                        const userPermissionsEndpoint = (userid) => `/api/users/read/permissions/all/${userid}`

                        let newPermission = undefined//{id: {id: permissionTitle.toString(), name: permission.toString(), defaultValue: false, checked:false}}
                        setInputs((prevStatus) => ({...prevStatus, ...newPermission}))
