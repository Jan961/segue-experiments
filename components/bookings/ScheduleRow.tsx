...
export const ScheduleRow = ({ date }: ScheduleRowProps) => {
  ...
  return (
    <div
      className="even:bg-black even:bg-opacity-5
      ...
      <div className={rowClass}>
        ...
        <div className="col-span-7 lg:col-span-8 grid grid-rows-auto gap-y-2 flex-grow pr-1">
          ...
          {date.RehearsalIds.map((id) => (
            <RehearsalDisplay key={id} date={date.Date} rehearsalId={id} />
          ))}
          {date.GetInFitUpIds.map((id) => (
            <GifuDisplay key={id} date={date.Date} gifuId={id} />
          ))}
          {date.OtherIds.map((id) => (
            <OtherDisplay key={id} date={date.Date} otherId={id} />
          ))}
          {total === 0 && (
            <div className="grid grid-cols-10">
              <CreateModal minimal date={date.Date} />
            </div>
          )}
        </div>
      </div>
