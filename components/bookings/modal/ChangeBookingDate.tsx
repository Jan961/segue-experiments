      ...
      <StyledDialog title={`Move Date: ${dateToSimple(booking.Date)}`} open={showModal} onClose={() => setShowModal(false)}>
        <form onSubmit={handleOnSubmit}>
          <FormInfo intent='WARNING' header="Warning">
            ...
            <p className="mt-2">Moving this booking requires space for <b>{existingDates.length}</b> dates</p>
          </FormInfo>
            ...
            <FormInfo intent='DANGER'>
              <ul>
                { unique(conflicts).map((bId) => (
                  <li key={bId}>Conflict: <b>{ venueDict[bookingDict[bId].VenueId].Name }</b></li>
                ))}
          ...
          <>
            <StyledDialog.FooterContainer>
              ...
              <StyledDialog.FooterContinue disabled={modalDisabled || !valid} submit>Change Date</StyledDialog.FooterContinue>
            </StyledDialog.FooterContainer>
          </>
        </form>
      </StyledDialog>
