...
interface BookingHoldProps {
    HoldAllocationId: number
}

export default function EditPromoterHold({HoldAllocationId}: BookingHoldProps) {
    ...
    useEffect(() => {
        fetch(`/api/bookings/holds/performanceHold/hold/${HoldAllocationId}`, {
            method: 'POST',
            headers: {
            ...
            .then(res => {
                setHold(res)
                    setInputs({
                        TicketHolderName: res.TicketHolderName,
                        ArrangedBy: res.ArrangedBy,
                        Comments: res.Comments,
                        HoldAllocationId: res.HoldAllocationId,
                        RequestedBy: res.RequestedBy,
                        Seats: res.Seats,
                        SeatsAllocated: res.SeatsAllocated,
                        TicketHolderEmail: res.TicketHolderEmail,
                        VenueConfirmationNotes: res.VenueConfirmationNotes
    ...
    function handleOnSubmit(e) {
        ...
        axios({
            method: 'POST',
            url: `/api/bookings/holds/performanceHold/update/${HoldAllocationId}`,
            data: inputs,
        })
            .then((response) => {
    ...
    return (
        <div className=" max-w-2/3 flex flex-row">
            <button
                className="bg-primary-blue text-white hover:bg-blue-400 active:bg-blue-600 font-bold text-sm px-3 py-1 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150"
                type="button"
