...
export const NumericSliderRange = ({
  min,
  max,
  value,
  isSlider,
  ...
  handleRangeChange,
  name,
}: NumericSliderRangeProps) => {
  if (isSlider) {
    return (
      <label>
        <div className="text-right inline-block text-xs mt-3">
          {label}: ({value[0]} to {value[1]})
        </div>
        <RangeSlider
          min={min}
          max={max}
          step={5}
          name={name}
          onInput={(x: [number, number]) => handleRangeChange(x, name)}
          className="my-4"
          value={value}
        />
      </label>
