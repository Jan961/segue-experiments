...
export default function CreatePanel({ finish }: CreatePanelProps) {
  ...
  if (mode === PanelMode.Booking) {
    return <CreateBookingPanel finish={finish} />;
  }

  if (mode === PanelMode.Gifu) {
    return <CreateGifuPanel finish={finish} />;
  }

  if (mode === PanelMode.Other) {
    return <CreateOtherPanel finish={finish} />;
  }

  if (mode === PanelMode.Gap) {
    return <GapPanel setGapVenueIds={setGapVenueIds} finish={finish} />;
  }

  if (mode === PanelMode.GapChoice) {
    return <GapChoicePanel reset={reset} gapVenues={gapVenues} />;
  }
