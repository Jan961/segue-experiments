...
export const BookingPanel = ({ bookingId }: BookingPanelProps) => {
  ...
  return (
    <form>
      {deleting && (
        <DeleteConfirmation title="Delete Booking" onCancel={() => setDeleting(false)} onConfirm={performDelete}>
      ...
      <div className={notFirst ? 'opacity-50' : ''}>
        ...
        <div
          className={classNames('flex flex-row justify-between gap-4', { '!justify-end': inputs.StatusCode === 'C' })}
        >
          {inputs.StatusCode !== 'C' && (
            <FormInputSelect
              inline
        ...
        <FormInputText value={inputs.Notes} onChange={handleOnChange} name="Notes" label="Notes" area />
      </div>
