...
export const ContactNotesEditor = ({ bookingContactNote, open, triggerClose, bookingId }: VenueContactsEditorProps) => {
  const [inputs, setInputs] = React.useState<Partial<BookingContactNoteDTO>>(
    bookingContactNote || { BookingId: bookingId },
  );
  ...
  return (
    <StyledDialog
      title={creating ? 'Create Contact Note' : 'Edit Contact Note'}
      open={open}
