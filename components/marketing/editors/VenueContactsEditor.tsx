...
export const VenueContactsEditor = ({ venueContact, open, triggerClose, venueId }: VenueContactsEditorProps) => {
  ...
  const roleList = [{ text: '-- Select Role --', value: undefined }, ...venueRoles.map((vr) => ({ text: vr.Name, value: vr.Id.toString() }))]

  return (
    <StyledDialog title={creating ? 'Create Venue Contact' : 'Edit Venue Contact'} open={open} onClose={() => triggerClose(false)}>
      <form onSubmit={handleOnSubmit}>
        ...
        <FormInputSelect name="RoleId" label="Role" value={inputs.RoleId} onChange={handleOnChange} options={roleList}/>
