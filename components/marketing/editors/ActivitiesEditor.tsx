...
export const ActivitiesEditor = ({ activity, open, triggerClose, types }: ActivitiesEditorProps) => {
  ...
  const options = [{ text: '-- Select Type --', value: '' }, ...types.map(x => ({ text: x.Name, value: x.Id.toString() }))]
  ...
  return (
    <StyledDialog title={creating ? 'Create Allocated Seat' : 'Edit Allocated Seat'} open={open} onClose={() => triggerClose(false)}>
      <form onSubmit={handleOnSubmit}>
        ...
        <FormInputSelect required name="ActivityTypeId" value={inputs.ActivityTypeId} label="Type" options={options} onChange={changeActivityType}/>
        <FormInputDate name="Date" value={inputs.Date} label="Date" onChange={handleOnChange}/>
        <FormInputCheckbox name="FollowUpRequired" value={inputs.FollowUpRequired} label="Follow Up Req" onChange={handleOnChange}/>
        <FormInputNumeric name="CompanyCost" label="Company Cost" value={inputs.CompanyCost} onChange={handleCompanyCostChange} />
        <FormInputNumeric name="VenueCost" label="Venue Cost" value={inputs.VenueCost} onChange={handleVenueCostChange} />
