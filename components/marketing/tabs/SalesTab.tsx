import { IconDefinition, faBook, faSquareXmark, faUser } from '@fortawesome/free-solid-svg-icons';
      ...
      <div className="grid grid-cols-12 gap-2 overflow-hidden">
        <div className="col-span-8 md:col-span-9 lg:col-span-10 mb-4">
          <Table className="mt-4 text-sm table-auto !min-w-0">
            <Table.HeaderRow>
              <Table.HeaderCell className="w-20 rounded-tl-lg">Week</Table.HeaderCell>
              <Table.HeaderCell className="w-20">Week of</Table.HeaderCell>
              <Table.HeaderCell className="w-20">Seats Sold (n)</Table.HeaderCell>
              <Table.HeaderCell className="w-20">Seats Sold (%)</Table.HeaderCell>
              <Table.HeaderCell className="w-20">Reserved (n)</Table.HeaderCell>
              <Table.HeaderCell className="w-20">Reserved (%)</Table.HeaderCell>
              <Table.HeaderCell className="w-20">Total Value</Table.HeaderCell>
              <Table.HeaderCell className="w-20">Value Change</Table.HeaderCell>
              <Table.HeaderCell className="w-20">Total Holds</Table.HeaderCell>
              <Table.HeaderCell className="w-20">Seats Change</Table.HeaderCell>
              <Table.HeaderCell className="w-20 rounded-tr-lg">Activity</Table.HeaderCell>
            </Table.HeaderRow>
              ...
              {bookingSales.map((sale, i) => {
                ...
                return (
                  <Table.Row id={sale.isFinal ? 'final' : null} key={sale.week}>
                    ...
                    <Table.Cell className={classNames(colorCode)}>{dateToSimple(sale.weekOf)}</Table.Cell>
                    <Table.Cell className="text-right">{numeral(sale.seatsSold).format('0,0')}</Table.Cell>
                    <Table.Cell className="text-right">
                      {((sale.seatsSold / sale.capacity) * 100).toFixed(2)} %
                    </Table.Cell>
                    <Table.Cell className="text-right">{sale.reservations}</Table.Cell>
                    <Table.Cell className="text-right">{numeral(sale.reserved).format('0,0')}</Table.Cell>
                    <Table.Cell className="text-right">
                    ...
                    <Table.Cell className="text-right">{sale.totalHolds}</Table.Cell>
                    <Table.Cell className="text-right">{seatsChange}</Table.Cell>
                    <Table.Cell>
                      <ul className="flex">
                        <Indicator
                          tooltip="Single Seat"
