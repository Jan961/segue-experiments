                        ...
                        <div className="mt-1 sm:col-span-2 sm:mt-0">
                            ...
                            >
                                ...
                                {activeSetTourDates.map((item) => (
                                    <option value={item.BookingId}>{dateService.getWeekDay(item.ShowDate)} {dateService.dateStringToSimple(item.ShowDate)} {(item.Venue.Name)} ({(item.Venue.Town)})</option>
                                ))}
                            ...
                            {loadedEmails.map((item) => (
                                <div key={item.id}>
                                    <button onClick={() => importEmail(item.Id)}>
                                        <span>
                                            {JSON.stringify(item)}
                                            {item.SetTour} {dateService.dateStringToSimple(item.Date)} Import
                                        </span>
                                    </button>
                                </div>
                            ))
