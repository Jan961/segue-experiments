...
export const dateToPicker = (dateToFormat: Date | string) => {
  ...
  return typeof dateToFormat === "object"
    ? dateToFormat.toISOString().slice(0, 10)
    : dateToFormat.slice(0, 10);
};
...
export const timeNow = () => {
  ...
  return (
    today.getHours().toFixed() +
    ":" +
    today.getMinutes().toFixed() +
    ":" +
    today.getSeconds().toFixed()
  );
};
...
export const getMonday = (inputDate) => {
  ...
  currentDateObj.setDate(
    currentDateObj.getDate() - ((currentDateObj.getDay() + 6) % 7)
  );
...
export const getSunday = (inputDate) => {
  const currentDateObj = new Date(inputDate);
  currentDateObj.setDate(
    currentDateObj.getDate() - ((currentDateObj.getDay() + 7) % 7) + 1
  );
  return currentDateObj;
};
