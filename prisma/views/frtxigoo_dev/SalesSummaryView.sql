SELECT
  `ScheduleView`.`TourId` AS `TourId`,
  `ScheduleView`.`FullTourCode` AS `FullTourCode`,
  `ScheduleView`.`ShowName` AS `ShowName`,
  `ScheduleView`.`TourStartDate` AS `TourStartDate`,
  `ScheduleView`.`TourEndDate` AS `TourEndDate`,
  `ScheduleView`.`TourWeekNum` AS `TourWeekNum`,
...
FROM
  (
    ...
    LEFT JOIN `frtxigoo_dev`.`ConversionRate` ON(
      `ScheduleView`.`TourId` = `frtxigoo_dev`.`ConversionRate`.`ConversionTourId`
      AND `frtxigoo_dev`.`Currency`.`CurrencyCode` = `frtxigoo_dev`.`ConversionRate`.`ConversionFromCurrencyCode`
    )
  )
WHERE
  `ScheduleView`.`EntryDate` >= `ScheduleView`.`TourStartDate`
ORDER BY
