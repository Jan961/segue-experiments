...
(
    SELECT
      `frtxigoo_dev2`.`SalesSetTotalsView`.`Seats`
    FROM
      `frtxigoo_dev2`.`SalesSetTotalsView`
    WHERE
      `frtxigoo_dev2`.`SalesSetTotalsView`.`SetBookingId` = `frtxigoo_dev2`.`Booking`.`BookingId`
      AND `frtxigoo_dev2`.`SalesSetTotalsView`.`SaleTypeName` = 'General Sales'
      AND `frtxigoo_dev2`.`SalesSetTotalsView`.`SetSalesFiguresDate` = (
        SELECT
          max(`SS1`.`SetSalesFiguresDate`)
        FROM
          `frtxigoo_dev2`.`SalesSet` `SS1`
        WHERE
          `SS1`.`SetBookingId` = `frtxigoo_dev2`.`SalesSetTotalsView`.`SetBookingId`
      )
  ) AS `SoldSeats`,
(
    SELECT
    ...
    WHERE
      `frtxigoo_dev2`.`SalesSetTotalsView`.`SetBookingId` = `frtxigoo_dev2`.`Booking`.`BookingId`
      AND `frtxigoo_dev2`.`SalesSetTotalsView`.`SaleTypeName` = 'General Reserved'
      AND `frtxigoo_dev2`.`SalesSetTotalsView`.`SetSalesFiguresDate` = (
        SELECT
          max(`SS1`.`SetSalesFiguresDate`)
        FROM
...
GROUP BY
  `frtxigoo_dev2`.`ProductionView`.`FullProductionCode`,
