SELECT
  ...
  `frtxigoo_dev2`.`VenueFamily`.`VenueFamilyName` AS `VenueFamily`,
  `MainCurrency`.`CurrencyCode` AS `VenueCurrencyCode`,
    ...
    (
      (
        (
          ...
          LEFT JOIN `frtxigoo_dev2`.`VenueFamily` ON(
            `frtxigoo_dev2`.`Venue`.`VenueVenueFamilyId` = `frtxigoo_dev2`.`VenueFamily`.`VenueFamilyId`
          )
        )
      ...
      LEFT JOIN `frtxigoo_dev2`.`Currency` `MainCurrency` ON(
        `MainCountry`.`CountryCurrencyCode` = `MainCurrency`.`CurrencyCode`
      )
    )
    LEFT JOIN `frtxigoo_dev2`.`Country` `DeliveryCountry` ON(
