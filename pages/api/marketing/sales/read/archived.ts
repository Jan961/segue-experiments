...
export default async function handle (req, res) {
  try {
    ...
    const commonData = formattedData.filter((x: TSalesView) => x.BookingId === bookingIds[0]).map(({ SetBookingWeekNum, SetTourWeekDate, SetIsFinalFigures }) => ({ SetBookingWeekNum, SetTourWeekDate, SetIsFinalFigures }))
    commonData.sort((a, b) => {
    ...
    const result: TSalesView[][] = commonData.map(({ SetBookingWeekNum, SetTourWeekDate, SetIsFinalFigures }) => formattedData.reduce(
      (acc, y) =>
