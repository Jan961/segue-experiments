  ...
  try {
    ...
    if (salesSet) {
      ...
      await prisma.$transaction(async (tx) => {
        await Promise.all(Comps.map(({ SetCompCompTypeId, SetCompSeats }) =>
          tx.setComp.upsert({
            where: {
        ...
        await Promise.all(Holds.map(({ SetHoldHoldTypeId, SetHoldSeats, SetHoldValue }) =>
          tx.setHold.upsert({
            where: {
        ...
        await Promise.all(Sales.map(({ SaleSaleTypeId, SaleSeats, SaleValue }) =>
          tx.sale.upsert({
            where: {
