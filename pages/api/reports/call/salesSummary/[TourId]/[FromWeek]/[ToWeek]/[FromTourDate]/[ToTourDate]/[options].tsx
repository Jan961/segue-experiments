...
export default async function handle(req, res) {
  ...
  if (req.query.options === 'data') {
    try {
      const result =
        await prisma.$queryRaw`SELECT * FROM \`SalesSummaryView\` WHERE \`TourId\` = ${TourId} AND \`ShowDate\` >= ${FromTourDate}  AND \`WeekDate\` BETWEEN ${FromWeek} AND ${ToWeek}  AND (IsNull(${FromTourDate})
    ...
    try {
      const result =
        await prisma.$queryRaw`SELECT DISTINCT WeekName, WeekDate, WeekCode FROM \`SalesSummaryView\` WHERE \`TourId\` = ${TourId} AND \`ShowDate\` >= ${FromTourDate}  AND \`WeekDate\` BETWEEN ${FromWeek} AND ${ToWeek}  AND (IsNull(${FromTourDate})
