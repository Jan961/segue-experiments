...
export default async function handle (req: NextApiRequest, res: NextApiResponse) {
  ...
  const jsonArray: TRequiredFields[] = data.filter(x => x.SaleTypeName === SALES_TYPE_NAME.GENERAL_SALES).map(({ BookingTourWeekNum, BookingFirstDate, VenueTown, VenueName, Value, VenueCurrencySymbol, SetBookingWeekNum, SetTourWeekDate, ConversionRate, SetIsCopy, SetBrochureReleased, BookingStatusCode, FinalFiguresValue, TotalCapacity, Seats, NotOnSalesDate }) => ({ BookingTourWeekNum, BookingFirstDate, VenueTown, VenueName, Value, VenueCurrencySymbol, SetBookingWeekNum, SetTourWeekDate, ConversionRate, SetIsCopy, SetBrochureReleased, BookingStatusCode, FinalFiguresValue, TotalCapacity, Seats, NotOnSalesDate }))
  const finalFormattedValues: TRequiredFieldsFinalFormat[] = jsonArray.map((x: TRequiredFields) => ({ ...x, Week: formatWeek(x.BookingTourWeekNum), FormattedValue: x.Value ? `${x.VenueCurrencySymbol}${x.Value}` : '', Value: x.Value || 0, FormattedSetTourWeekNum: formatWeek(x.SetBookingWeekNum), FormattedFinalFiguresValue: x.FinalFiguresValue ? `${x.VenueCurrencySymbol}${x.FinalFiguresValue}` : '', Day: moment(x.BookingFirstDate).format('dddd'), Date: x.BookingFirstDate, Town: x.VenueTown, Venue: x.VenueName }))
