...
import { emailService } from 'services/emailServiceLegacy';
...
function Register() {
  ...
  const validationSchema = Yup.object().shape({
    ...
    email: Yup.string().required('Your Email is required, to allow login and notifications'),
    ...
    business_type: Yup.string().required('What kind of business are you will you use this software with'),

    // Account Payment
    card_number: Yup.string().notRequired(),
    card_expiration_date: Yup.string()
      .required('Expiration Date is required')
      .matches(/^(0[1-9]|1[0-2])?([0-9]{2})$/, 'Must be Date in 0112 (Month)(Year)')
    ...
    terms_agree: Yup.boolean().oneOf([true], 'You must accept the terms and conditions'),
  });
  ...
  async function createAccount(user) {
    try {
      const response = await axios.post('/api/account/create/', createPayload(user));
          ...
          <div className="w-full max-w-3xl space-y-8">
            <div>
              ...
              <p className="mt-2 text-center text-sm text-gray-600">
                If you have been invited to Join use the link in the email to create an account linked to your
                production company
              </p>
            </div>
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <div className="flex flex-row items-center justify-between h-24 border-y-2 border-gray-500 my-4">
                ...
                <span className="text-xl text-gray-500 font-bold ">Pay Monthly</span>
              </div>
              <div>
